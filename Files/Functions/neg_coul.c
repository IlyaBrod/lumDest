...
{//printf("%d\n",i);
 for(j=0;j<dimx;j++)
 {
  ...
  ior[i*dimx+j]=(int)chi;//irr[i*dimx+j]=255-ior[i*dimx+j];
  chi=(unsigned char)fgetc(fio);
  iov[i*dimx+j]=(int)chi;//irv[i*dimx+j]=255-iov[i*dimx+j];
  chi=(unsigned char)fgetc(fio);
  iob[i*dimx+j]=(int)chi;//irb[i*dimx+j]=255-iob[i*dimx+j];
 }
}
